...
(define tests
  (test-suite
   ...
   #;
   (test-suite
    "test the collection functions in parse-type.rkt"
    (test-case
        "first"
      ;; (define-type B (
      (define-values (r1 r2 r3)
        (phase1-phase0-eval
         (begin
           ;; (define r1 (parse-type (quote-syntax Number)))
           ;; (define r2 r1)
           ;; (define r3 r1)
           (define-values (r1^ r2 r3) (parse-for-effects #'AAA (cons null (quote-syntax Number))))
           (define r1 (filter Name? r1^))
           #`(values (list #,@r1) (list #,@r2) #,r3))))
      (check-equal? r1 null)
      (check-equal? r2 null)
      (check-equal? r3 #t))
    (test-case
      ...
      (define-values (r1 r2 r3)
        (phase1-phase0-eval
         (begin
           ...
           ;; (define r2 r1)
           ;; (define r3 r1)
           (define-values (r1^ r2 r3) (parse-for-effects #'B (cons (list #'a) (quote-syntax (Listof (B a))))))
           (define r1 (filter Name? r1^))
           #`(values (list #,@r1) (list #,@r2) #,r3))))
      (check-equal? r1 null)
      (check-equal? r2 null)
