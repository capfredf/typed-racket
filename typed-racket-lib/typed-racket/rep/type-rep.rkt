...
;; keep track of types generated by TypeConstructors
(define generated-types-tbl (make-hash))
...
(lazy-require ["../types/substitute.rkt" [subst-all make-simple-substitution]]
              ["../types/utils.rkt" [instantiate-poly]])

(provide (struct-out user-defined-type-op)
         (struct-out struct-type-op))
...
(struct TypeConstructor (real-trep-constr arity kind*? productive?)
  ...
  (lambda (me . args)
    ...
    (define ret
      (match real-trep-constr
        [(? procedure?)
         (apply real-trep-constr
                (cond
                  [kind*?
        ...
        [(user-defined-type-op vars type)
         (subst-all (make-simple-substitution vars args)
                type)]
        [(struct-type-op sty)
         (instantiate-poly sty args)]))

    ;; we do not have to keep track of a base type or a type in the args.
    (unless (or (Base? ret) (member ret args))
      (hash-set! generated-types-tbl ret (cons me args)))
    ret))

(define (make-type-op type-maker [arity 1] [productive #t] #:kind*? [kind*? #f])
  (when (and (procedure? type-maker) (not (object-name type-maker)))
    (error 'make-type-op "only named procedures are allowed"))
  ...
  (match type-or-type-op
    [(struct* TypeConstructor ([arity arity]
     ...
     (define all-stars (if kind*? (append mandatory-stars (list "*..."))
                           mandatory-stars))
     (format "(-> ~a *)" (string-join all-stars))]
    [else "*"]))


(define (lookup-kind id)
  (free-id-table-ref kind-env id #f))

(define (register-type-constructor! name type-constr)
  (when (equal? (TypeConstructor-productive? type-constr) 'undecided)
        (error 'drunk "hihi ~a~n" name))
